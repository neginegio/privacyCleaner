...
def compare_workbooks(actual_path: Path, expected_path: Path, max_diffs: int) -> list[str]:
...
def main() -> int:
    ...
    findings = processor.scan(args.source)
    candidates = [finding for finding in findings if finding.detection_kind == "確認候補"]
    if candidates and not args.approve_candidates:
        print(f"BLOCKED_REVIEW_REQUIRED count={len(candidates)}")
        for finding in candidates[: args.max_diffs]:
    ...
    with tempfile.TemporaryDirectory(prefix="ExcelPrivacyExpected_") as tmp:
        actual_path = processor.convert(args.source, findings, Path(tmp))
        diffs = compare_workbooks(actual_path, args.expected, args.max_diffs)
        if diffs:
