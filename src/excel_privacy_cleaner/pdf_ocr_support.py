...
def ocr_line_word_spans(
    words: list[tuple[Any, ...]],
) -> tuple[tuple[str, tuple[tuple[int, int, tuple[float, float, float, float]], ...]], ...]:
    """Group OCR words into reading-order lines, at *word* granularity --
    deliberately not subdivided per character.

    Used to reconcile a text-layer PDF page's detections (which can have
    unreliable per-character positions when the embedded text is itself an
    OCR layer generated by third-party scanning software -- see the
    "GlyphLessFont" case in pdf_processor.py) against a redaction rect
    grounded in this module's own, self-consistent OCR pass over the
    rendered page image.

    Per-character positions (as `_build_normalized_ocr` produces, by evenly
    subdividing each word's box across its characters) are *not* reliable
    enough for that job: Tesseract's own word segmentation for Japanese
    (a language with no space-delimited words) routinely merges several
    semantic words into one recognized token -- observed on a real document
    merging a full 27-character sentence into a single "word" -- and evenly
    subdividing a block that coarse drifts far past any individual
    character's real position. A word's own bounding box, by contrast, is
    Tesseract's directly measured value and reliable at that granularity.
    Each returned line is (concatenated_normalized_text, spans), where
    spans is a tuple of (start_offset, end_offset, word_rect) so a caller
    can look up which original word(s) a matched substring range came from
    and union their real bounding boxes, without any synthetic subdivision.
    """
    ...
    lines: list[tuple[str, tuple[tuple[int, int, tuple[float, float, float, float]], ...]]] = []
    for key in line_order:
        parts: list[str] = []
        spans: list[tuple[int, int, tuple[float, float, float, float]]] = []
        offset = 0
        for word in line_words[key]:
            normalized = _normalize_general(str(word[4]))
            if not normalized:
                continue
            rect = (float(word[0]), float(word[1]), float(word[2]), float(word[3]))
            spans.append((offset, offset + len(normalized), rect))
            parts.append(normalized)
            offset += len(normalized)
        lines.append(("".join(parts), tuple(spans)))
    return tuple(lines)
